...
summary:  
---
= Protection des données 3-2-1 pour VMware avec le plug-in SnapCenter et la BlueXP backup and recovery pour les machines virtuelles
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[%collapsible%open]
====
L'objectif de cette solution est de démontrer la protection des données des machines virtuelles exécutées dans VMware vSphere et situées sur des magasins de données NFS hébergés par NetApp ONTAP.  Cette solution suppose que les composants suivants sont configurés et prêts à l’emploi :
